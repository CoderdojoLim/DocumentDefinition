...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="фильм">
		<p class="MovieTitle"><font size="3">
		<xsl:apply-templates select="./заглавие"/><xsl:text> </xsl:text>
		<xsl:call-template name="looper">
			<xsl:with-param name="iterations" select="@рейтинг"/>
		</xsl:call-template> 
		</font>
		</p>
		<p>
			Director: <xsl:apply-templates select="директор"/><br/>
			<xsl:text>Cast: </xsl:text>
			<xsl:for-each select="актерскийсостав">
				<xsl:apply-templates select="./Имя"/><xsl:text> </xsl:text><xsl:apply-templates select="./фамилия"/>
